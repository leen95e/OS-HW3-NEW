...
int append_stats(char* buf, threads_stats t_stats, time_stats tm_stats) {
    ...
    offset += sprintf(buf + offset, "Stat-Req-Arrival:: %ld.%06ld\r\n",
                      (long)tm_stats.task_arrival.tv_sec, (long)tm_stats.task_arrival.tv_usec);
    offset += sprintf(buf + offset, "Stat-Req-Dispatch:: %ld.%06ld\r\n",
                      (long)tm_stats.task_dispatch.tv_sec, (long)tm_stats.task_dispatch.tv_usec);
    offset += sprintf(buf + offset, "Stat-Log-Arrival:: %ld.%06ld\r\n",
                      (long)tm_stats.log_enter.tv_sec, (long)tm_stats.log_enter.tv_usec);
    offset += sprintf(buf + offset, "Stat-Log-Dispatch:: %ld.%06ld\r\n",
                      (long)tm_stats.log_exit.tv_sec, (long)tm_stats.log_exit.tv_usec);

    pthread_mutex_lock(&t_stats->lock);
    ...
    pthread_mutex_unlock(&t_stats->lock);
    return offset;
}

// Builds the Stat-Thread-* block only, used to answer UDP statistics pings
// (there is no associated job, so no Stat-Req-*/Stat-Log-* fields).
int format_thread_stats(threads_stats t_stats, char *buf) {
    int offset = 0;
